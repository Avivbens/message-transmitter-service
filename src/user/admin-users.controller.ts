...
export class AdminUsersController {
    private readonly logger: Logger = new Logger(AdminUsersController.name)

    constructor(private readonly usersService: UsersService, private jwtService: JwtService) {}

    // TODO add pagination
    @Get()
    getUsers(@Query() queryParams: GetUsersDto, @CurrentUser() user: User, @Ip() ip: string) {
    ...
    @Patch(':id')
    async loginViaUser(@Param('id') id: string, @CurrentUser() adminUser: User, @Ip() ip: string) {
        ...
        if (user.isAdmin) {
            ...
            throw new BadRequestException('Admin user cannot login via another admin')
        }
    ...
    updateUserProfile(
        ...
        this.logger.log(`Updating user ${id}, admin is ${user.email} - ${user._id}, ip is ${ip}`)
        return this.usersService.updateForAdmin(updatedUser, id)
    }

    @Delete('delete-user/:id')
    async deleteUser(@Param('id') id: string, @CurrentUser() user: User, @Ip() ip: string) {
        ...
        this.logger.warn(`Deleting user ${id}, admin is ${user.email} - ${user._id}, ip is ${ip}`)
